...
// ✅ Accept children so App.js can inject <Cases/> / <CreateCase/>
export default function Dashboard({ onLogout, user, children }) {
  ...
  // ✅ add closeCase helper (you already call it in the table)
  ...
  if (children) {
    // ✅ When App.js provides a page (Cases/CreateCase), render it here
    content = (
      <section className="col" style={{ maxWidth: 1100, margin: '12px auto' }}>
        {children}
      </section>
    );
  } else {
    // Fallback to your original internal screens if no children were passed
    content = (
      <>
        {screen === 'create' && (
